...
class EnCore(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lstm = EnCore.PMHCLSTM(
            mhclen=mhclen,
            minpep=minpep,
            enclen=enclen,
            hidlen=hidlen,
            ncells=layers)
        self.anchorBlock = EnCore.AnchorBlock(
            mhclen=mhclen,
            minpep=minpep,
            enclen=enclen,
            hidlen=hidlen,
            layers=layers)
        self.condenser = EnCore.Condenser(
            mhclen=mhclen,
            minpep=minpep,
            enclen=enclen,
            hidlen=hidlen,
    ...
    def save(self, fp, tl=False):
        ...
        for idx, (k,v) in enumerate(self.state_dict().items()):
            if not tl or k in EnCore.tllayers():
    ...
    def load(fp):
        ...
        if len(layers) == len(EnCore.tllayers()):
            tl = fp
