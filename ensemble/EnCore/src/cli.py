...
helper functions for loading EnCore models, accelerating them on GPUs,
...
def _parseModel(args : argparse.Namespace) -> argparse.Namespace:
    """
    Replace "el" with "EnCore_el" and "im" with "EnCore_im".
    Then replace "EnCore_el" and "EnCore_im" with relative paths
    ...
    if args.models.strip().lower() == "el":
        args.modelname = "EnCore_EL"
        models = elmodels
    elif args.models.strip().lower() == "im":
        args.modelname = "EnCore_IM"
        models = immodels
    else:
        args.modelname = "EnCore"
...
def _loadModels(args : argparse.Namespace) -> EnCore:
    """
    Wrapper for EnCore.load with optional verbose printing.
    ...
    if args.models is None:
        ...
        for model in args.models:
            ...
            models.append(EnCore.load(model))
            if args.verbose:
                print("done")
    return models
...
def parseArgs(train):
    ...
    parser = argparse.ArgumentParser(
        ("Train EnCore from scratch or just the final and penultimate layers"
         " using a specified base model for transfer learning") if train else
         "Predict pMHC presentation or immunogenicity with EnCore")
